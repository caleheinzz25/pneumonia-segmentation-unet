...
\begin{alertblock}{\footnotesize Sistem yang Dibangun}
  \scriptsize Penelitian ini membangun sistem \textbf{CAD} (\textit{Computer-Aided Detection}) berbasis U-Net + sCSE + EfficientNet-B3 mengikuti kerangka CRISP-DM, dilengkapi Grad-CAM dan antarmuka web Gradio yang dapat diakses via internet melalui Cloudflare Tunnel.
\end{alertblock}
\end{frame}
...
\begin{columns}[c]
  ...
  \begin{column}{0.62\textwidth}
    \begin{block}{\footnotesize 6 Fase CRISP-DM pada Penelitian Ini}
      \begin{enumerate}\scriptsize
        \item \textbf{Business Understanding}: Kebutuhan sistem CAD sebagai alat bantu skrining awal penunjang keputusan dokter.
        \item \textbf{Data Understanding}: Dataset RSNA Pneumonia (26.684 CXR DICOM), analisis \textit{class imbalance}.
        \item \textbf{Data Preparation}: Konversi DICOM ke PNG 8-bit, PSPNet Dual Lung Masking, augmentasi data (8 teknik).
  ...
  \begin{column}{0.65\textwidth}
    \begin{block}{\footnotesize Rincian Tahapan Alur Penelitian}
      \begin{itemize}\scriptsize
        ...
        \item \textbf{Preprocessing}: Normalisasi Min-Max \& segmentasi ROI paru PSPNet.
        ...
        \item \textbf{Evaluasi Model}: Pengujian pada data validasi 1.336 pasien (20\%) \& Confusion Matrix tingkat piksel.
...
\begin{columns}[c]
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{block}{\footnotesize Activity Diagram Inferensi Medis}
      \begin{itemize}\tiny
        \item \textbf{Alur Logika}: Input Rontgen $\rightarrow$ Preprocessing PSPNet $\rightarrow$ Inferensi U-Net + sCSE $\rightarrow$ Generasi Heatmap Grad-CAM $\rightarrow$ Output 4-Panel \& Severity Report.
      \end{itemize}
    \end{block}
  \end{column}
\end{columns}
  ...
  \begin{column}{0.53\textwidth}
    \begin{block}{\footnotesize Confusion Matrix Piksel}
      ...
      \begin{tabular}{lcc}
        ...
        \midrule
        \textbf{Act. Normal} & \textbf{TN=312.284.691} (97,75\%) & \textbf{FP=7.198.145} (2,25\%) \\
        \textbf{Act. Pneumonia} & \textbf{FN=9.995.562} (32,51\%) & \textbf{TP=20.745.986} (67,49\%) \\
        \bottomrule
      \end{tabular}
      
      \vspace{0.1cm}
      *Catatan: Precision piksel $\approx$74,24\%, Recall piksel $\approx$67,49\%
    \end{block}
    \begin{exampleblock}{\footnotesize Signifikansi Klinis}
      \scriptsize Recall 70,82\% (image-level) meminimalkan risiko \textit{false negative} medis. Nilai Specificity 97,75\% dari confusion matrix piksel membuktikan model sangat jarang salah mengenali jaringan sehat sebagai pneumonia.
    \end{exampleblock}
  \end{column}
\end{columns}
\end{frame}
...
\begin{frame}{ABLATION STUDY --- ANALISIS KOMPONEN}{Pengaruh Komponen Utama terhadap Performa Model (Analisis Kualitatif)}
\centering
\scriptsize
\begin{tabular}{p{3.5cm}p{4.5cm}p{4.5cm}}
  ...
  Atensi sCSE & $\downarrow$ Ketelitian batas arsiran & Hasil evaluasi literatur (Roy et al., 2018) \\
  Unified Focal Loss & $\downarrow$ Kemampuan mencari area pneumonia & Nilai Recall yang tinggi (0,7082) \\
  Pemotongan Paru (PSPNet) & $\uparrow$ Salah deteksi di luar paru-paru & Sesuai hasil penelitian Teixeira et al. (2021) \\
  AMP + Grad. Accumulation & $\uparrow$ Waktu latihan, memori GPU tidak cukup & Pelatihan tidak bisa berjalan lanpa ini \\
  Augmentasi Gambar & $\downarrow$ Kemampuan membaca gambar baru & Kurva loss latihan dan validasi tetap stabil \\
  \bottomrule
\end{tabular}

\vspace{0.1cm}
\begin{alertblock}{\footnotesize Bukti Terkuat Transfer Learning}
  \scriptsize Penurunan \textit{Val Loss} dari 0,3212 (Epoch 12, encoder frozen) ke 0,2755 (Epoch 46) membuktikan bahwa bobot ImageNet pada EfficientNet-B3 sangat berpengaruh terhadap kemampuan model mengenali fitur rontgen paru-paru.
\end{alertblock}
\end{frame}
...
\begin{columns}[T]
  ...
  \begin{column}{0.24\textwidth}
    ...
    Peta aktivasi INFERNO colormap
  \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
  \begin{column}{0.50\textwidth}
    \begin{block}{\footnotesize INFERNO Medical Thermal Colormap}
      \centering
      \tiny
      \begin{tabular}{lcl}
        \toprule
        ...
        \midrule
        \rowcolor{yellow!30} \textbf{Kuning/Putih} & $0,8 - 1,0$ & Pusat \textit{hotspot} lesi infeksi berat \\
        \rowcolor{orange!25} \textbf{Oranye} & $0,6 - 0,8$ & Aktivasi tinggi / margin lesi transisi \\
        \rowcolor{red!20} \textbf{Merah Tua} & $0,3 - 0,6$ & Aktivasi sedang / perilesional \\
        \rowcolor{purple!15} \textbf{Hitam/Ungu} & $0,0 - 0,3$ & Jaringan paru sehat / background \\
        \bottomrule
      \end{tabular}
    \end{block}
  \end{column}
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{block}{\footnotesize Fitur Gradio Web App}
      \begin{itemize}\tiny
        ...
        \item Inferensi cepat \textbf{1--3 detik} per citra (GPU RTX 3070).
      \end{itemize}
    \end{block}
  \end{column}
